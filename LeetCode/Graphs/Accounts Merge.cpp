...
class Solution
{
    unordered_map<string, int> visited_email;
    vector<vector<string>> ans;
    unordered_map<string, vector<string>> adj;

    void dfs(string &email)
    {
        if (visited_email.count(email))
            return;
        visited_email[email]++;
        ans.back().push_back(email);
        for (auto &i : adj[email])
        {
            dfs(i);
        }
    }
...
public:
    vector<vector<string>> accountsMerge(vector<vector<string>> &accounts)
    {
        for (auto &i : accounts)
        {
            ...
            {
                adj[i[k]].push_back(i[k - 1]);
                adj[i[k - 1]].push_back(i[k]);
            }
        }

        for (auto &i : accounts)
        {
            if (!visited_email.count(i[1]))
            {
                ans.push_back({i[0]});
                dfs(i[1]);
