...
void leftView(Node *root, int currHeight, int &maxHeight, vector<int> &result)
{
    if (!root)
        return;
    ...
    leftView(root->left, currHeight + 1, maxHeight, result);
    leftView(root->right, currHeight + 1, maxHeight, result);
}
vector<int> leftView(Node *root)
{
    int maxHeight = -1;
    vector<int> result;
    leftView(root, 0, maxHeight, result);
    return result;
}