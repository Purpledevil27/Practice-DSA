...
void help(vector<vector<char>> &board, int n, int m, int i, int j, char color, int t, int **visited)
{
    ...
    // Checking if already visited or not and finding the difference of the indexing.
    if (visited[i][j] != 0)
    {
    ...
    // Marking visited and indexing orderwise
    visited[i][j] = ++t;

    help(board, n, m, i + 1, j, color, t, visited);
    help(board, n, m, i, j + 1, color, t, visited);
    help(board, n, m, i - 1, j, color, t, visited);
    help(board, n, m, i, j - 1, color, t, visited);
}
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            if (visited[i][j] == 0 && !ans)
            {
                ...
                help(board, n, m, i, j, color, 0, visited);
            }
        }
    }
    return ans;
